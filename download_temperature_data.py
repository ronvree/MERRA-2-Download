...
COLLECTION_NAME = 'inst1_2d_asm_Nx'
...
def get_data_from_url(url: str, key='T2M', filter_locations: bool=True):
...
def f(date: datetime.date, key='T2M'):
    ...
    np.save(f'data/MERRA2_M2I1NXASM_5.12.4_{key}_{date.strftime("%Y-%m-%d")}.npy', data)


def get_processed_dates(key='T2M'):
    # Read the data obtained
    dates = []
    for fn in os.listdir('data'):
        date = datetime.datetime.strptime(fn, f'MERRA2_M2I1NXASM_5.12.4_{key}_%Y-%m-%d.npy').date()
        dates.append(date)
    return dates
